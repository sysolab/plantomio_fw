...
find_latest_firmware_version() {
    ...
    for dir in "$DEBUG_FIRMWARE_DIR"/*; do
      if [ -d "$dir" ]; then
        ...
        if [[ $base_name =~ waterBee_([0-9]+\.[0-9]+\.[0-9]+)_debug_merged ]]; then
          ...
          debug_versions+=("$version")
        fi
      fi
    done
    
    # Sort versions and get the latest
    if [ ${#debug_versions[@]} -gt 0 ]; then
      IFS=$'\n' debug_sorted=($(sort -V <<<"${debug_versions[*]}"))
      unset IFS
  ...
  if [ -d "$RELEASE_FIRMWARE_DIR" ]; then
    ...
    if [ ${#release_versions[@]} -gt 0 ]; then
      IFS=$'\n' release_sorted=($(sort -V <<<"${release_versions[*]}"))
      unset IFS
  ...
  echo -e "${GREEN}Using firmware version: $FIRMWARE_VERSION${NC}"
...
prepare_release_files() {
  ...
  if [ -d "$DEBUG_DIR" ]; then
    ...
    echo -e "${YELLOW}Debug firmware directory not found, skipping debug variant.${NC}"
  fi
  ...
  if [ -d "$RELEASE_DIR" ]; then
    ...
    echo -e "${YELLOW}Release firmware directory not found, skipping release variant.${NC}"
  fi
